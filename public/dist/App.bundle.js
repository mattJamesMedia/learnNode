...
function ajaxHeart(e) {
  ...
  _axios2.default.post(this.action).then(function (res) {
    ...
    // console.log(res.data.hearts.length);
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      if (res.data.length) {
        _dompurify2.default.sanitize(searchResults.innerHTML = searchResultsHTML(res.data));
        return;
      }
      searchResults.innerHTML = _dompurify2.default.sanitize('<div class="search__result">No result for ' + _this.value + ' found</div>');
    }).catch(function (err) {
      console.error(error);
    });
  });
